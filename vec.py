...
def equal(u,v):
    ...
    for d in v.f:
        if d in u.f:
            if(v.f[d]!=u.f[d]): return False
        else: return False

    for d in u.f:
        if d in v.f:
            if (u.f[d] != v.f[d]): return False
        else:
            return False
...
def add(u,v):
    ...
    """
    assert u.D == v.D
    c= Vec(u.D,{d:(v.f[d] if (d in v.f) else 0) +(u.f[d] if (d in u.f) else 0) for d in v.D})
    d1 = Vec(u.D,{})
    for d in c.f:
        if (d not in u.f) and (d not in v.f): print()
        else : d1.f[d]=c.f[d]
    return d1
    pass
...
def dot(u,v):
    ...
    """
    assert u.D == v.D
    return sum((v.f[d] if (d in v.f) else 0) *(u.f[d] if (d in u.f) else 0) for d in v.D)
    pass
